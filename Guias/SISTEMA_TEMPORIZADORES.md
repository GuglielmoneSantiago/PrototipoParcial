...
### **Ejemplo 1: Temporizador de Invencibilidad**

```python
class Player:
    def __init__(self, x: int, y: int):
        # ... código existente ...
        self.invincibility_timer = DurationTimer(120)  # 2 segundos de invencibilidad
        
    def take_damage(self, damage: int):
        """Recibir daño con invencibilidad"""
        if not self.invincibility_timer.is_active():
            self.health -= damage
...
### **Ejemplo 2: Temporizador de Regeneración de Salud**

```python
class Player:
    def __init__(self, x: int, y: int):
        # ... código existente ...
        self.health_regen_timer = IntervalTimer(300)  # Cada 5 segundos
        
    def update(self):
        """Actualizar jugador"""
        # ... código existente ...
        
        # Regenerar salud cada 5 segundos si está por debajo del 80%
        if self.health < self.max_health * 0.8:
            if self.health_regen_timer.update():
                self.health = min(self.max_health, self.health + 5)
                print("Salud regenerada: +5")
```

### **Ejemplo 3: Temporizador de Habilidad Especial**

```python
class Player:
    def __init__(self, x: int, y: int):
        # ... código existente ...
        self.special_ability_timer = CooldownTimer(1800)  # 30 segundos de cooldown
        
    def use_special_ability(self):
        """Usar habilidad especial"""
        if self.special_ability_timer.trigger():
            # Ejecutar habilidad especial
            print("¡Habilidad especial activada!")
            # Aquí iría el código de la habilidad
        else:
            print(f"Habilidad en cooldown: {self.special_ability_timer.current_cooldown // 60} segundos")
            
    def update(self):
        """Actualizar jugador"""
        # ... código existente ...
        self.special_ability_timer.update()
```

### **Ejemplo 4: Temporizador de Efecto Temporal**

```python
class Game:
    def __init__(self):
        # ... código existente ...
        self.rain_timer = SimpleTimer(1800)  # Lluvia por 30 segundos
    ...
    def start_rain(self):
        ...
        print("¡Empieza a llover!")
        
    def update(self):
        """Actualizar lógica del juego"""
        # ... código existente ...
        
        # Actualizar temporizador de lluvia
        if self.rain_timer.update():
            self.is_raining = False
            print("La lluvia ha parado")
            
        # Efectos de la lluvia
        if self.is_raining:
            # Regenerar lagunas más rápido
            for lagoon in self.lagoons:
                lagoon.water_level = min(100, lagoon.water_level + 0.5)
```
